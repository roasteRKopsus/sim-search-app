...
#                     iccid = str(row[mapped['iccid']]).strip() if pd.notna(row[mapped['iccid']]) else None
#                     msisdn = str(row[mapped['msisdn']]).strip() if pd.notna(row[mapped['msisdn']]) else None
#                     imsi = str(row[mapped['imsi']]).strip() if pd.notna(row[mapped['imsi']]) else None
...
#                 upload_record.row_count = inserted
#                 db.session.commit()
#                 flash(f"Success: {inserted} unique rows uploaded!", 'success')
#             except Exception as e:
#                 db.session.rollback()
#                 flash(f"Error: {str(e)}", 'danger')
#                 if os.path.exists(filepath):
...
#     print('iniiii -----',input_val)
#     number = extract_longest_number(input_val)
#     print('ini 2  -',number.type())
#     if not number:
#         flash("No number found", 'danger')
#         return redirect(url_for('index'))

#     results = []
#     queries = [
#         SimData.sn.contains(number),
#         SimData.iccid.contains(number),
#         SimData.msisdn.contains(number),
#         SimData.imsi.contains(number)
#     ]
...
def search():
    ...
    # q = session.query(DatasetEntry).filter(
    #         or_(
    #             DatasetEntry.sn.ilike(ilike_pattern),
    #             DatasetEntry.iccid.ilike(ilike_pattern),
    #             DatasetEntry.msisdn.ilike(ilike_pattern),
    #             DatasetEntry.imsi.ilike(ilike_pattern),
    #         )
    #     )
...
#     query = SimData.query.join(UploadedFile)
#     if date_filter:
#         try:
#             filter_date = datetime.strptime(date_filter, '%Y-%m-%d')
#             query = query.filter(db.func.date(UploadedFile.uploaded_at) == filter_date.date())
#         except:
#             pass
#     sims = query.order_by(UploadedFile.uploaded_at.desc()).paginate(page=page, per_page=50)
#     return render_template('data.html', sims=sims, date_filter=date_filter)
